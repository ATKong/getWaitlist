    ...
    `,e.style.disabled=!0,document.getElementById("getWaitlistErrorContainer"));a.innerHTML="",a.parentElement.parentElement.style.display="none"}function gw_primary_cta_is_not_loading_anymore(e){var a=document.getElementById("primaryCTA");a.innerHTML=e,a.style.disabled=!1}async function gw_get_waiter_obj(){gw_waiter_obj=null,gw_clear_errors(),gw_primary_cta_is_loading();var e=gw_waitlist_container.querySelector("[name=email]").value;let a;await fetch(`${gw_backend_url}waiter?email=${encodeURIComponent(e)}&waitlist_id=`+encodeURIComponent(gw_waitlist_obj.id),{Method:"GET",Headers:{Accept:"application/json","Content-Type":"application/json"}}).then(e=>(a=e.status,e.json())).then(e=>{if(399<a)return gw_handle_error_json("get_waiter_obj",e,e.error_string||"An error has occurred.","Check Status");gw_waiter_obj=e,"WIDGET_1"==gw_widget_type?gw_make_waitlist_widget("SUCCESS"):"WIDGET_2"==gw_widget_type&&gw_make_waitlist_widget_type_2("SUCCESS")})}async function gw_post_waiter_obj(){gw_clear_errors(),gw_primary_cta_is_loading();var e={waitlist_id:gw_waitlist_obj.id,referral_link:window.location.href,heartbeat_uuid:gw_heartbeat_uuid,widget_type:gw_widget_type},a=gw_waitlist_container.querySelector("[name=email]")?.value,a=(a&&(e.email=a),gw_waitlist_container.querySelector("[name=first_name]")?.value),a=(a&&(e.first_name=a),gw_waitlist_container.querySelector("[name=last_name]")?.value),a=(a&&(e.last_name=a),gw_waitlist_container.querySelector("[name=phone]")?.value&&gw_waitlist_container.querySelector("[name=countrycode]")?.value?gw_waitlist_container.querySelector("[name=countrycode]").value+gw_waitlist_container.querySelector("[name=phone]").value:null),a=(a&&(e.phone=a),Array.from(gw_waitlist_container.querySelectorAll(".gw_question")));a.sort(function(e,a){parseInt(e.id.slice(12)),parseInt(a.id.slice(12))}),e.answers=a.map(e=>({question_value:e.name,answer_value:e.value}));let t;await fetch(gw_backend_url+"waiter",{method:"POST",headers:{Accept:"application/json","Content-Type":"application/json"},body:JSON.stringify(e)}).then(e=>(t=e.status,e.json())).then(e=>399<t?gw_handle_error_json("post_waiter_obj",e,e.error_string||"An error has occurred.",gw_signup_button_title):(gw_waiter_obj=e,!gw_waitlist_obj.uses_signup_verification&&gw_waiter_obj.priority>gw_waitlist_obj.statistics.current_signups&&(gw_waitlist_obj.statistics.current_signups=gw_waiter_obj.priority),gw_waitlist_obj.uses_leaderboard?get_leaderboard():void("WIDGET_1"==gw_widget_type?gw_make_waitlist_widget("SUCCESS"):"WIDGET_2"==gw_widget_type&&gw_make_waitlist_widget_type_2("SUCCESS"))))}async function get_leaderboard(){let a;await fetch(gw_backend_url+`waitlist/${gw_waitlist_id}/leaderboard`,{method:"GET",headers:{Accept:"application/json","Content-Type":"application/json"}}).then(e=>(a=e.status,e.json())).then(e=>{200===a&&(gw_leaderboard=e,"WIDGET_1"==gw_widget_type?gw_make_waitlist_widget("SUCCESS"):"WIDGET_2"==gw_widget_type&&gw_make_waitlist_widget_type_2("SUCCESS"))})}function gw_danger_select(e){e.field_key&&("phone"==e.field_key&&gw_waitlist_container.querySelectorAll("[name=countrycode]").forEach(e=>e.classList.add("danger_select")),gw_waitlist_container.querySelectorAll(`[name="${e.field_key}"]`).forEach(e=>e.classList.add("danger_select")))}function gw_copyButton(){try{navigator.clipboard.writeText(gw_waiter_obj.referral_link),document.getElementById("getWaitlistgw_copyButton").innerHTML="Copied!"}catch(e){gw_error_handler(e)}}function gw_make_waitlist_widget(e){gw_waitlist_container.style.backgroundColor=gw_background_color;let a;if(""===gw_title&&""===gw_success_title||"CHECK"!==e?""!==gw_title&&"SUBMIT"===e?a=gw_title||"Sign up for "+gw_waitlist_name:""!==gw_success_title&&"SUCCESS"===e&&(a=gw_success_title||"Successfully signed up for "+gw_waitlist_name):a="Check your status",a?gw_waitlist_container.innerHTML=`
        ...
        `:gw_waitlist_container.innerHTML="","SUBMIT"===e){let e="",a=(gw_waitlist_obj.uses_firstname_lastname&&(e=`
                <div class="${gw_flex}${gw_flex_row}${gw_justify_center}${gw_w_full}${gw_mt_2}">
                    ...
                    ${gw_waitlist_obj.remove_widget_headers?"":`<span style="color: ${gw_font_color}" class="${gw_text_sm}${gw_font_semibold}">Last Name</span>`}
            ...
            `),""),t=(["EMAIL","EMAIL_OR_PHONE","EMAIL_AND_PHONE"].includes(gw_waitlist_obj.required_contact_detail)&&(a=gw_make_email_field()),"");if(["PHONE","EMAIL_OR_PHONE","EMAIL_AND_PHONE"].includes(gw_waitlist_obj.required_contact_detail)){let a=[];gw_country_codes.forEach(e=>a.push(`<option value="${e.value}" style="color: ${gw_font_color};background-color:${gw_background_color}">${e.label}</option>`)),t=`
            ...
            `}let l="";if(gw_waitlist_obj.questions&&0<gw_waitlist_obj.questions?.length){var _=gw_waitlist_obj.remove_widget_headers?"":gw_mt_2;l+=`
            ...
            `;for(let e=0;e<gw_waitlist_obj.questions.length;e++)l+=gw_make_question_answer_field(e);l+="</div>"}gw_waitlist_container.innerHTML+=`
            <div class="${gw_w_full}">
        ...
        `}else if("CHECK"===e){let e="";["EMAIL","EMAIL_OR_PHONE","EMAIL_AND_PHONE"].includes(gw_waitlist_obj.required_contact_detail)&&(e=gw_make_email_field()),gw_waitlist_container.innerHTML+=`
            <div class="${gw_w_full}">
                <div class="${gw_flex}${gw_flex_col}${gw_justify_between}${gw_items_center}${gw_rounded_3xl}${gw_w_full}">
                    ${e}
                </div>
            </div>
        `}else if("SUCCESS"===e){let e="",a=(e=gw_waiter_obj.removed_date?"Offboarded!":!gw_waitlist_obj.uses_signup_verification||gw_waiter_obj.verified?gw_waiter_obj.priority:`
                    ...
                    `,"");if(gw_waitlist_obj.hide_counts){let e="";e=gw_waiter_obj.removed_date?"Offboarded!":gw_waitlist_obj.uses_signup_verification&&!gw_waiter_obj.verified?`This waitlist requires you to verify your email. To join, click the link we sent to ${gw_waiter_obj.email}.`:"You are on the waitlist!",a=`
                ...
                <div class="${gw_px_4}${gw_py_5}${gw_rounded_md}${gw_border}${gw_overflow_hidden}${sm_gw_p_6}${gw_sm_w_1_2}${gw_text_center}" style="${gw_color_border_background()}">
                    ...
                    <dd class="${gw_mt_1}${gw_font_semibold}${gw_waiter_obj.removed_date?gw_text_2xl:gw_text_3xl}" style="color: ${gw_font_color}; background-color: ${gw_background_color}">
                        ${e}
                    </dd>
                </div>
        ...
        `,gw_waitlist_obj.uses_leaderboard&&gw_leaderboard){let _=[];gw_leaderboard?.leaderboard.forEach((e,a)=>{var t=e.first_name&&e.last_name?e.first_name+" "+e.last_name:"",l=e.first_name,e=e.email;_.push(`
            ...
            `}}_={SUBMIT:["gw_post_waiter_obj()",gw_signup_button_title],CHECK:["gw_get_waiter_obj()","Check Status"]};if("SUCCESS"!==e)gw_waitlist_container.innerHTML+=`
            <div class="${gw_mt_4}${gw_text_center}">
                <button
                    id="primaryCTA"
                    style="background-color: ${gw_button_color}; color: ${gw_button_font_color}"
        ...
        `;else{var l=[],n=(gw_waitlist_obj.show_email_on_widget&&l.push("email"),gw_waitlist_obj.show_facebook_on_widget&&l.push("facebook"),gw_waitlist_obj.show_instagram_on_widget&&l.push("instagram"),gw_waitlist_obj.show_linkedin_on_widget&&l.push("linkedin"),gw_waitlist_obj.show_reddit_on_widget&&l.push("reddit"),gw_waitlist_obj.show_telegram_on_widget&&l.push("telegram"),gw_waitlist_obj.show_twitter_on_widget&&l.push("twitter"),gw_waitlist_obj.show_whatsapp_on_widget&&l.push("whatsapp"),[]);let t=1==l.length?gw_w_full:2==l.length?gw_w_1_2:3==l.length?gw_w_1_3:4<=l.length?gw_w_1_4:"";for(let a=0;a<l.length;a++){let e="";a+1!==l.length&&(l.length<=4&&(e=gw_mr_2),5!=l.length&&6!=l.length||2!==a&&(e=gw_mr_2),7!=l.length&&8!=l.length||3!==a&&(e=gw_mr_2)),"twitter"==l[a]&&n.push(`
                    <div class="${t} ${e}">
                        <button
                            style="background-color: ${gw_button_color}; color: ${gw_button_font_color}"
                            class="${gw_text_center}${gw_items_center}${gw_px_4}${gw_py_2}${gw_font_bold}${gw_text_sm}${gw_rounded_md}${gw_font_medium}${focus_gw_ring_2}${gw_border}${gw_border_transparent}${gw_w_full}${hover_gw_opacity_80}${gw_transition}${gw_duration_300}"
        ...
        `}gw_waitlist_container.innerHTML+=gw_error_handling_area,"SUBMIT"===e?gw_waitlist_container.innerHTML+=`
            <div class="${gw_mt_4}${gw_text_center}${gw_text_sm}" style="color: ${gw_font_color}">
                Signed up before? <span class="${hover_gw_cursor_pointer}${gw_font_bold}" onClick="gw_make_waitlist_widget('CHECK')">Check your Status<span>
            </div>
        ...
        `;else if("SUCCESS"===e){let e="",a="";gw_waiter_obj.removed_date?e="Offboarded!":gw_waitlist_obj.uses_signup_verification&&!gw_waiter_obj.verified?e=`This waitlist requires you to verify your email. To join, click the link we sent to ${gw_waiter_obj.email}.`:a=(e=gw_waitlist_obj.hide_counts?"You are on the Waitlist! Refer people to move up: ":`You are #${gw_waiter_obj.priority} in line! Refer people to move up: `,`<div class="${gw_flex}${gw_flex_row}">${gw_waiter_obj.referral_link}</div>`),gw_waitlist_container.innerHTML=`
        ...
        `}gw_waitlist_container.innerHTML+=gw_widget_2_error_handling_area,gw_waitlist_obj.uses_waitlist_widget_branding&&(gw_waitlist_container.innerHTML+=gw_widget_2_branded_footer())}function gw_make_question_answer_field(e){var t=gw_waitlist_obj.questions[e];let l=gw_mt_2;if(0!==e||gw_waitlist_obj.remove_widget_headers||(l=" "),t&&t.answer_value){let a=[];return t.answer_value.forEach(e=>a.push(`<option value="${e}" style="color: ${gw_font_color};background-color:${gw_background_color}">${e}</option>`)),`
            <div class="${gw_mt_1}">
                <select
                    ...
                    onchange="gw_modify_select_on_change(this, '${gw_font_color}')"
                >
                    <option value="" style="color: ${gw_font_color};background-color:${gw_background_color}">${t.question_value}</option>
                    ${a.join("")}
                </select>
            </div>
        `}return`
